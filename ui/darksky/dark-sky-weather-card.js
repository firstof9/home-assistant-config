...
class DarkSkyWeatherCard extends LitElement {
    ...
    get windIcons() {
        return {
            ...
            var quadrant = Math.round(Number(this.config.entity_wind_bearing) / 45);
    ...
    get current() {
        ...
        var windBearing = this.config.entity_wind_bearing ? isNaN(this._hass.states[this.config.entity_wind_bearing].state) ? this._hass.states[this.config.entity_wind_bearing].state : this.windIcons[(Math.round((this._hass.states[this.config.entity_wind_bearing].state / 360) * 16))] : 0;
